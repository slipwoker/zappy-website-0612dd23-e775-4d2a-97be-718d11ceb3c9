...
if (phoneHeaderBtn) {
  phoneHeaderBtn.addEventListener('click', () => {
...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    // This is just for any additional client-side validation or UX
    const submitBtn = contactForm.querySelector('.form-submit-btn');
    if (submitBtn) {
      submitBtn.innerHTML = '<span>שולח...</span><i class="fas fa-spinner fa-spin"></i>';
    }
  });
}
...
const animatedElements = document.querySelectorAll('.card-default, .menu-item, .feature-card, .value-card, .team-member, .testimonial-card');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "My Coffee משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '0612dd23-e775-4d2a-97be-718d11ceb3c9',
                    name: data.name || '',
                    email: data.email || '',
